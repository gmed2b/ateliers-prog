def is_mail(str_arg: str) -> (int, int):
    ...
    if len(str_arg) == 0:
        ...
        if len(local_domain) < 2:
            ...
            if len(local) < 1 or (local[0] == '.' or local[-1] == '.') or ('..' in local):
                is_valid = (0, 1)
            elif '.' not in domain:
                is_valid = (0, 4)
            elif len(domain) < 1 or (domain[0] == '.' or domain[-1] == '.') or (
...
def test():
    test_cases = [
        ...
        ("bisgambiglia_paul@univ-corse_P.fr", (0, 3)),
        ("@univ-corse.fr", (0, 1))
    ]
    for i, (input_str, expected_output) in enumerate(test_cases):
        result = is_mail(input_str)
        if result == expected_output:
            print(f"Test case `{input_str}` PASSED (got {result} as expected {expected_output})")
        else:
